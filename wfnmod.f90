...
30  continue
  ...
  subroutine evalwfn(m,mesh,usexcdm)
     use param
     !     
     ...
     !
     !     Edited by Kyle R. Bryenton March 2025
     !     Now supports XCDM
     !
     ! ---
     ! 
     ...
     if (istat /= 0) call error('evalwfn','could not allocate memory for BR b parameter',2)
  ...
  subroutine propts_xcdm(m,nr,r,rho,b)
  ...
  subroutine bhole(rho,quad,hnorm,b,uinv)
    ...
    real*8, intent(out) :: b
    real*8, optional, intent(out) :: uinv
...
1002 format(' ','bhole: newton algorithm fails to initialize!')
  ...
  subroutine edisp(m,a1,a2,z_damp,egauss,usec9,xdm_damping)
    !
    !     Calculates the XDM/XCDM dispersion energy, forces, stresses
    !
    !     Edited by K R Bryenton 2024-03-21
    !     Added support for Z-damping routine for XDM(Z)
    !
    type(molecule), intent(in) :: m
    real*8, intent(in)         :: a1, a2, z_damp, egauss
    logical, intent(in)        :: usec9
    integer, intent(in)        :: xdm_damping
 
    integer :: i, j, k, k1, k2
    real*8  :: d, atpol(m%n), fac, c6, c8, c10, zinv, zinvz, rc, rvdw
    ...
    select case (xdm_damping)
    case(1) ! BJ Damping
        write (iout,'("# i  j       dij            C6               C8               C10              Rc           Rvdw")')
    case(2) ! Z Damping
        write (iout,'("# i  j       dij            C6               C8               C10              Zinv         Zinvz")')
    case default
        write(iout, '(2X,A,F12.6)') 'ERROR: Neither BJ nor Z damping detected. --- xdm_damping = ', xdm_damping
        stop 1
    end select
    e = 0d0
    f = 0d0
    ...
    do i = 1, m%n
       ...
       do j = i, m%n
          ...
          ! Calculate damping contributions depending on if BJ or Z damping
          select case (xdm_damping)
          case(1) ! BJ Damping
             rc = (sqrt(c8/c6) + sqrt(sqrt(c10/c6)) + sqrt(c10/c8)) / 3.D0
             rvdw = a1 * rc + a2
             damp6 = rvdw**6.d0
             damp8 = rvdw**8.d0
             damp10 = rvdw**10.d0
          ...
          ! Different output depending on damping type
          select case (xdm_damping)
          case(1) ! BJ Damping
    ...
    ! Calculate C9 coefficients if requested
    if (usec9) then
